...
export default function RenderGuidaForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = await fetch("/api/renderguida", {
        method: "POST",
  ...
  if (status === "success") {
    return (
      <div className="bg-white rounded-[2rem] p-8 lg:p-10 shadow-xl border border-gray-100 text-center">
        ...
        <h3 className="text-2xl font-bold text-accent mb-4">Richiesta ricevuta!</h3>
        <p className="text-accent/70 leading-relaxed mb-6">
          Grazie. <strong>Ti richiamo personalmente io</strong> &mdash; nessun call center &mdash; per
          fissare insieme il tuo incontro. Riceverai una chiamata da questo numero:
        </p>
        <a
          href={WHATSAPP_URL}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 text-lg font-bold text-accent-2 hover:underline"
        >
          <Phone size={20} /> 351 89 71 482
        </a>
        <p className="text-sm text-accent/50 mt-4 italic">
